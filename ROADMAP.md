...
- **[avoid] A dedicated "Goals Check" / goal-reprioritization feature.** Built and removed (v0.18, briefly). Reordering goal bullets is a two-second manual edit (the Goals editor), and the real need — a *fuzzy* "am I working on the right things?" reassurance — is already served by the weekly review's Big Rocks Check and by just asking the assistant in chat. Every attempt to make it a feature needed a reliable todo→goal-bullet mapping (tags, or semantic inference), which is overhead without enough payoff for an occasional, deliberately-fuzzy check. Keep it fuzzy; keep it conversational; don't build a surface for it.
