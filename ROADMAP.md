...
- **[done ✓ v0.9.0] PDF reading for the documents shelf** (S/M) — close the gap `read_document` admits; Claude reads PDFs natively as base64 `document` blocks.
...
- **[done ✓ v0.15.0] Vault writes via draft/approve** (L) — extend preserve-and-splice + the ✓/✗ draft flow to arbitrary notes ("file these takeaways into [[Team Notes]]"). Never auto-apply to non-todo notes regardless of size — every note write is a reviewed draft.
- **[done ✓ v0.16.0] Review journal** (M) — the weekly review writes a short visible `reviews/…md` entry the user owns and browses in Obsidian. *Guardrail nuance:* the no-dates rule protects task metadata; a dated journal entry is a journal (Obsidian-native). Dates never touch `todo.md`. Decision pending.
- **[done ✓ v0.18.0] Goals Check — seasonal reprioritization *within* goals** (S) — a "◎ Check" affordance on the Goals panel (and a plain chat ask) triggers a scripted, conversational pass: the assistant gauges each goal *bullet's* "pull" (active Top 5 / Urgent / Today todos advancing it, mapped by meaning), surfaces only the ≤3–4 **mismatches** as questions (a low bullet pulling lots of work → move up; a high bullet with no pull → north-star, keep or let drift; a done bullet → strike), then reorders bullets **within** their section + prunes via `write_goals` into the normal ✓/✗ draft. Reprioritization is *within* a section, not re-ranking sections. No bespoke UI — reuses `read_goals`/`write_goals`, the chat, the goals editor, and the draft/approve flow. Deliberately *not* a dashboard: no always-on meters, no %, no charts. Design partnered with Fable; altitude (bullet-within-section, not section-level) set by the user's real notepad.
- **[done ✓ v0.17.0] Vault Module 3 — careful cleanup / reorganization** (L, MVP) — `vault_overview` + `find_duplicate_notes` (read-only survey: folder tree, dates, size, inbound/outbound links, empty/stale flags) → `move_note` (file into folders, created on apply) + `trash_note` (recoverable move to `.bigrocks/trash`, **never** a hard delete). Every op is STAGED into the existing ✓/✗ review pipeline (new `note-move`/`note-trash` rows; trash rows are visibly destructive), nothing touches disk until Apply. Applied ops are recorded in a per-batch manifest → one-click **↶ Undo cleanup**. Safety: protected paths (`todo.md`, memory, `.obsidian`, attachments) refused at stage *and* execute; `withinRoot`; no-overwrite; `#keep` tag opt-out; linked-note trash guard; per-note sha256 preconditions (a cloud-sync mid-review skips the op, keeps the row). *Phase 2 (later):* `rename_note` + `[[wikilink]]` relink rewriting, `merge_notes`, path-style link fixes for moves — the riskiest code (rewriting third-party notes) ships last.
