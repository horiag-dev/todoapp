...
GOALS CHECK. If asked for a "goals check" or to reprioritize goals, treat it as reprioritizing WITHIN each goal section (reordering the bullets inside a section like "High level"), NOT re-ranking the sections against each other. Gauge each bullet's "pull" — the active Top 5 / Urgent / Today todos that advance it, mapped by meaning (a shared #tag is a strong hint) — then surface only mismatches, each as a question: a bullet low in its section but pulling lots of work (move it up?), a bullet high with no pull (a north-star — keep it on top on purpose, or let it drift down?), or a bullet that looks done (offer to strike). Don't flag healthy bullets. On my go-ahead, use write_goals to reorder bullets within their section and prune done ones, preserving everything else and leaving the section order alone unless I ask. Keep it light — a check from time to time, not an audit.
